...
# SYSTEM PREFERENCES
	...
	# Security & Privacy
		...
		# Security & Privacy Workflow
			...
			# Confirm that SecureToken is enabled once you have enabled FV. Please be aware that the user would need to have FV enabled.
				...
				# Another way to add the secure token - BEST OPTION FOR NEW USERS
				sysadminctl -secureTokenOn USERNAME -password - -adminUser ADMIN_USERNAME -adminPassword -
		...
		# Security & Privacy Terminal Actions
			...
			# Print the FileVault status key
	...
	# Users & Groups
		...
		# Users & Groups Workflow
			...
			# Create new user from Terminal pre 10.15
			...
			# Create new user from terminal post 10.15
		...
		# Users & Groups Terminal Actions
			...
			# List users without underscore (system)
...
# SYSTEM
	...
	# System Workflow
		...
		# Disk Permissions
			...
			diskutil list
			sudo diskutil unmountDisk /dev/DISK_IDENTIFIER
...
# BROWSER
	...
	# Browser Terminal Actions
		...
		defaults write ~/Desktop/com.apple.Safari.plist HomePage https://www.google.com.au

		# Set Safari homepage
		defaults write ~/Desktop/com.apple.Safari.SandboxBroker.plist Homepage https://www.google.com.au
